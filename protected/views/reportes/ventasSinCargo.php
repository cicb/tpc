        ...
        <div class="span7">
            ...
            foreach ($evento->funciones as $funcion) {
                foreach ($funcion->zonas as $zona) {
                    $aforo = Lugares::model()->count("EventoId = '$eventoId' AND FuncionesId = '$funcionesId' AND ZonasId =".$zona->ZonasId);

                    echo "<div class='panel-head'>".$zona->ZonasAli." <div class='pull-right badge badge-success'>AFORO: $aforo</div> </div>";
                    $zonas=$this->widget('bootstrap.widgets.TbGridView', array(
                        'id'=>'taquilla-grid',
...
<div class="span5 remark" style="float:right">
    ...
    $taquillas=$this->widget('bootstrap.widgets.TbGridView', array(
        'id'=>'taquilla-grid',
        'emptyText'=>'No se encontraron coincidencias',
        'dataProvider'=>$model->getReporteTaquilla($eventoId,$funcionesId,$desde,$hasta,$cargo=false),
        'summaryText'=>'',
        'columns'=>array(
            array(
        ...
        $puntos=$this->widget('bootstrap.widgets.TbGridView', array(
            ...
            'emptyText'=>'No se encontraron coincidencias',
            'dataProvider'=>$model->getReporte($eventoId,$funcionesId,$desde,$hasta,$cargo=false,'NORMAL','and t3.FuncPuntosventaId<>t.PuntosventaId'),
            'summaryText'=>'',
...
$especiales=$this->widget('bootstrap.widgets.TbGridView', array(
    ...
    'dataProvider'=>$model->getReporte($eventoId,$funcionesId,$desde,$hasta,$cargo=false,
...
Yii::app()->clientScript->registerScript('sumatorias',"
    ...
    $('#canales-venta-grid table').append('<tr class=\'panel-head\'><td colspan=\'3\' ></td></tr><tr class=\'panel-head panel-foot\' > <td ><b>Subtotales. </b></td> <td style=\"text-align:right\">".($vendidos['taquilla']+$vendidos['puntos'])."</td><td style=\"text-align:right\">$".number_format($subtotal['taquilla']+$subtotal['puntos'],2)."</td> ')

    $('#boletos-especiales-grid table').append('<tr class=\'panel-head\'><td colspan=\'3\' style=\'text-align:center\'>TOTAL</td></tr><tr class=\'panel-head panel-foot\'> <td ><b>Total: </b></td> <td style=\"text-align:right\">".(array_sum($vendidos))."</td><td style=\"text-align:right\">$".number_format(array_sum($subtotal),2)."</td> ')
    "
...
$this->widget(
    ...
    array(
        ...
        'datasets' => array(
            array(
                "value" => $vendidos['especiales'],
                "color" => "#d35400",
                "label" => "Boletos duros y cortesias"
                ),
            array(
                "value" => $vendidos['taquilla'],
                "color" => "#2ecc71",
                "label" => "Taquilla"
                ),
            array(
                "value" => $vendidos['puntos'],
                "color" => "#2980b9",
                "label" => "Puntos venta"
                ),
            array(
                "value" => $vendidos['descuentos'],
                "color" => "#34495e",
                "label" => "Descuentos"
                )
            ),
    ...
    <?php 
// Yii::import('application.vendor.phpexcel.JPhpExcel');
// $xls = new JPhpExcel('UTF-8', false, 'My Test Sheet');
// $xls->addArray($puntos->getData());
// $xls->generateXML('Puntos');
    ?>
