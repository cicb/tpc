                ...
                var foroMapIntId = '<?php echo $model->ForoMapIntId; ?>';
					 if ($(this).val()!='' && $(this).val()!=null) {
							 if ($.inArray($(this).val().split('.').pop(),ext)==-1) {
								...
								fd.append('prefijo', 'mapaChico_');
                                fd.append('foroId', foroId);
                                fd.append('foroMapIntId', foroMapIntId);
                                //console.log(fd);
								$.ajax({
										url: '<?php echo Yii::app()->createUrl('distribuciones/subirImagenMapaChico')?>',
												type: 'POST',
												data: fd,
												...
												success: function(data){ 
												    //console.log(data);
														if (data) {
																$('#area-imagen-chica img').attr('src','../imagesbd/'+data);
...
$("#select-sub-zona").change(function(){
    ...
    if(zona!="" && subzona!=""){
        ...
        $.ajax({
            url:'<?php echo $this->createUrl('distribuciones/getCoordenadaMapaChico') ?>',
            type:'post',
            dataType:'json',
            beforeSend:function(){
...
$("#ver-coordenadas").click(function(){
    ...
    var funcionId = '<?php echo $funcionId?>';
    $.ajax({
        url:'<?php echo $this->createUrl('distribuciones/getCoordenadasMapaChico') ?>',
            type:'post',
            dataType:'json',
            beforeSend:function(){
                $("#cargando").html("Cargando Coordenadas");
...
$("#guardar-coordenada").click(function(){
    ...
    if(zona!="" && subzona!=""){
        $.ajax({
                url:'<?php echo $this->createUrl('distribuciones/guardarCoordenadasMapaChico') ?>',
                type:'post',
                dataType:'json',
                beforeSend:function(){
...
$("#eliminar-coordenada").click(function(){
    ...
    if(zona!="" && subzona!=""){
        ...
        if (del == true) {
            $.ajax({
                url:'<?php echo $this->createUrl('distribuciones/deleteCoordenadaMapaChico') ?>',
                type:'post',
                beforeSend:function(){
